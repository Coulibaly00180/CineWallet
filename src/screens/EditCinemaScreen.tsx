...
import { EditCinemaScreenProps } from '@/types/navigation';
import { imageManager } from '@/utils/imageUtils';

/**
 * Écran pour éditer un cinéma existant
 *
 * Fonctionnalités :
 ...
 * - Gestion des logos avec possibilité de modification
 */
export default function EditCinemaScreen({ navigation, route }: EditCinemaScreenProps) {
  const { cinemaId } = route.params;
  const { items: cinemas, update } = useCinemas();

  // Trouver le cinéma à éditer
  ...
  const [website, setWebsite] = useState(cinema?.website || '');
  const [phone, setPhone] = useState(cinema?.phone || '');
  const [primaryColor, setPrimaryColor] = useState(cinema?.primaryColor || '#1976d2');
  const [secondaryColor, setSecondaryColor] = useState(cinema?.secondaryColor || '#e3f2fd');
  const [qrFormat, setQrFormat] = useState(cinema?.qrFormat || '');
  const [notes, setNotes] = useState(cinema?.notes || '');
  const [logoUri, setLogoUri] = useState<string>(cinema?.logoUri || '');

  // États UI
  const [loading, setLoading] = useState(false);
  const [errors, setErrors] = useState<Record<string, string>>({});

  // Rediriger si le cinéma n'existe pas
  useEffect(() => {
    if (!cinema) {
      Alert.alert('Erreur', 'Cinéma introuvable.', [
  ...
  const handleSave = async () => {
    ...
    try {
      await update(cinemaId, {
        ...
        country: cinema.country, // Conserver le pays existant
      });

      Alert.alert('Succès', 'Cinéma modifié avec succès.', [
        { text: 'OK', onPress: () => navigation.goBack() }
      ]);
    } catch (error) {
      Alert.alert('Erreur', 'Impossible de modifier le cinéma. Vérifiez que le nom n\'est pas déjà utilisé.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.header}>
        <IconButton icon="movie-edit" size={32} iconColor="#1976d2" />
        <Text variant="headlineSmall" style={styles.headerTitle}>Modifier le cinéma</Text>
        <Text variant="bodyMedium" style={styles.headerSubtitle}>
          Modifiez les informations de {cinema.name}
        </Text>
      </View>
